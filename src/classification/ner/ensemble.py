...
class EnsembleModel(Model):
    ...
    def train(self):
        ...
        logging.info("Training complete, saving to {}/{}/{}.pkl".format(self.basedir, self.path, self.path))
    ...
    def test(self, corpus):
        ...
        for i, p in enumerate(pred):
            if p:
                sentence = self.corpus.get_sentence(self.offsets[i][0])
                eid = sentence.tag_entity(self.offsets[i][1], self.offsets[i][2], self.etype, source=self.path)
                results.entities[eid] = sentence.entities.get_entity(eid, self.path)
        return results

    def load_data(self, corpus, flist, etype, mode="train", doctype="all"):
        """
        ...
        for did in corpus.documents:
            ...
            for sentence in corpus.documents[did].sentences:
                if "goldstandard_{}".format(etype) in sentence.entities.elist:
                    sentence_eids = [e.eid for e in sentence.entities.elist["goldstandard_{}".format(etype)]]
                else:
                    sentence_eids = []
                for e in sentence.entities.elist["results/ensemble_chemdner_dev"]:
