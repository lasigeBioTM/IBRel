...
from gpro_corpus import GproCorpus
from ddi_corpus import DDICorpus
from chebi_corpus import ChebiCorpus
from corpus import Corpus
from taggercollection import TaggerCollection
from matcher import MatcherModel
from simpletagger import SimpleTaggerModel, BiasModel, feature_extractors
from stanfordner import StanfordNERModel
from results import ResultsNER, ResultSetNER
...
corenlp_dir = "stanford-corenlp-full-2015-01-30/"
'''
TODO:
better evaluation - compare boundaries, statistics, etc - improve Results class
test with semeval, chemdner, chemdner patents
relation extraction
'''
...
def main():
    ...
    parser.add_argument("actions", default="classify", nargs='+',
...
considered when coadministering with megestrol acetate.''',
    ...
    if "load_corpus" in options.actions:
        if corpus_format == "chemdner":
            ...
            if options.goldstd == "chemdner_traindev":
                ...
                corenlpserver = StanfordCoreNLP(corenlp_path=corenlp_dir,
                                        properties=corenlp_dir + "default.properties")
        ...
        corpus.save()
        if corpus_ann and "test" not in options.goldstd:
        ...
        if "annotate" in options.actions:
            ...
            corpus.load_annotations(corpus_ann)
            corpus.save()
        if "write_goldstandard" in options.actions:
        ...
        # training options
        if "train" in options.actions:
            #model = SimpleTaggerModel(options.models)
            ...
            #corpus.save()
        if "train_matcher" in options.actions:
            model = MatcherModel(options.models)
            model.train(corpus)
        if "train_multiple" in options.actions:
        ...
        if "test_multiple" in options.actions:
